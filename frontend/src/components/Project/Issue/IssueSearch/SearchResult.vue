<template>
  <div
    @click="selectResult"
    class="flex items-center px-3 py-1 transition duration-100 rounded cursor-pointer select-none hover:bg-backgroundLight"
  >
    <j-icon :size="24" class="flex-shrink-0" :name="issue.type"></j-icon>
    <div class="pl-4">
      <div class="text-textDark text-15">{{ issue.title }}</div>
      <div class="text-xs uppercase text-textMedium">
        {{ `${issue.type}-${issue.id}` }}
      </div>
    </div>
  </div>
